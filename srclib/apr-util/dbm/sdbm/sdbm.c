                         ...
                         apr_int32_t flags, apr_fileperms_t perms, apr_pool_t *p)
{
    ...
    *pdb = NULL;
     ...
     */
#ifdef __VMS
    if ((flags & APR_CREATE) == 0) {
        if ((status = apr_stat(&finfo, dirname, APR_FINFO_NORM, p)) != APR_SUCCESS)
            goto error;

        flags |= (((finfo.fab_rfm == FAB$C_VAR) ||
                   (finfo.fab_rfm == FAB$C_VFC)) ? APR_CTX_REC : APR_CTX_STM);
    }
    else {
        flags |= APR_CTX_STM;
    }
...
#ifdef __VMS
    if ((flags & APR_CREATE) == 0) {
        if ((status = apr_stat(&finfo, pagname, APR_FINFO_NORM, p)) != APR_SUCCESS)
            goto error;

        flags &= ~(APR_CTX_REC | APR_CTX_STM);
        flags |= (((finfo.fab_rfm == FAB$C_VAR) ||
                   (finfo.fab_rfm == FAB$C_VFC)) ? APR_CTX_REC : APR_CTX_STM);
    }
    else {
