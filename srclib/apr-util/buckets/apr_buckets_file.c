                                     ...
                                     apr_size_t *len, apr_read_type_e block)
{
    ...
    *len = (filelength > a->read_size) ? a->read_size : filelength;
    ...
    filelength -= *len;
#else
    /* For VMS, if we access a file via record mode
     * (i.e. Variable length and VFC record format)
     * then we perform multiple reads to get the whole
     * buffer.  One read per record.  All other formats
     * are opened in a stream context and we should
     * go thru this "while" loop only once.
     */
    rlen = *len;
    bcount = 0;
    c = buf;
    ...
    e->length = *len = bcount;  /* The real count in this bucket */
    if (rv == APR_EOF) {        /* If we hit <EOF> */
        filelength = 0;         /* Then the bytes remaining = 0 */
    }
    else {                      /* Otherwise */
        filelength -= bcount;   /* Decrement to estimate bytes remaining */
    }
#endif
