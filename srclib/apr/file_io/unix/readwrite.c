...
#ifdef __VMS
APR_DECLARE(int) apr_vms_var_file_size(apr_file_t *thefile, int len)
{
    int fd = thefile->filedes;
    int rlen = 0;
    int bcount = 0;
    char *buf;
    ...
    while ((rlen = read(fd, buf, len)) > 0) {
        bcount += rlen;                 /* Count the bytes */
        len -= rlen;
    }

    lseek(fd, 0, 0);                    /* Rewind to beginning */
                                       ...
                                       apr_size_t *nbytes)
{
    ...
    while (rv == 0 && size > 0) {
        if (thefile->bufpos >= thefile->dataRead) {
                ...
                if (thefile->is_pipe && thefile->is_socket)
                    bytesread = recv (thefile->filedes, thefile->buffer, thefile->bufsize, 0);
                else
                    if (thefile->is_pipe)
                        bytesread = read (thefile->filedes, thefile->buffer, thefile->pipe_buffer_size);
                    else
                        bytesread = read (thefile->filedes, thefile->buffer, thefile->bufsize);

                /*
                ** If this is a mailbox pipe and we've received an EOF on the first read
                ** then lets read again.
                */
                if (bytesread == 0 &&
                    thefile->is_pipe && ! thefile->is_socket && ! thefile->skipped_first_pipe_eof)
                    {
                    thefile->skipped_first_pipe_eof = TRUE;
                    continue;
                    }
#else
...
APR_DECLARE(apr_status_t) apr_file_read(apr_file_t *thefile, void *buf, apr_size_t *nbytes)
{
    ...
    if (thefile->buffered) {
        ...
        do {
...
#ifdef __VMS
            if (thefile->is_pipe && thefile->is_socket)
                rv = recv (thefile->filedes, buf, *nbytes, 0);
            else
#endif
            ...
            /*
            ** If this is a mailbox pipe and we've received an EOF on the first read
            ** then lets turn this into a buffered read, just in case the caller is
            ** is trying to read one byte at a time.
            */
            if (rv == 0 &&
                thefile->is_pipe && ! thefile->is_socket && ! thefile->skipped_first_pipe_eof)
                {
                thefile->buffer = apr_palloc (thefile->pool, thefile->pipe_buffer_size);
                thefile->skipped_first_pipe_eof = TRUE;
                thefile->buffered = TRUE;
        ...
        if (rv == -1 &&
            ...
            if (thefile->is_pipe && thefile->is_socket)
                {
                apr_socket_t thesock;

                thesock.pool = thefile->pool;
                thesock.socketdes = thefile->filedes;
                thesock.timeout = thefile->timeout;

                arv = apr_wait_for_io_or_timeout (NULL, &thesock, 1);
                }
            else
                arv = apr_wait_for_io_or_timeout (thefile, NULL, 1);
#else
