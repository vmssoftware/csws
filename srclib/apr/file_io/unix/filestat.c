...
#endif
#ifdef __VMS
    finfo->fab_rfm = info->st_fab_rfm;
    finfo->fab_rat = info->st_fab_rat;
    finfo->fab_fsz = info->st_fab_fsz;
    finfo->fab_mrs = info->st_fab_mrs;
#endif
}
...
#ifdef __VMS
extern int apr$stat(char *, stat_t *, ...);
extern int BRC_stat(const char *, stat_t *);
extern int BRC_lstat(const char *, stat_t *);
#endif
                                   ...
                                   apr_int32_t wanted, apr_pool_t *pool)
{
    ...
    if (wanted & APR_FINFO_LINK)
        srv = BRC_lstat(fname, &info);
    else
        srv = BRC_stat(fname, &info);
#else
    if (wanted & APR_FINFO_LINK)
...
#ifdef __VMS
    if (srv != 0 && *fname == '/' && strrchr (fname, '/') == fname) /* Error and looks like device */
        srv = apr$stat((char *) fname, &info);
#endif
