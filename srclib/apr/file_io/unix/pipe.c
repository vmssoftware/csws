/* Licensed to the Apache Software Foundation (ASF) under one or more
 * contributor license agreements.  See the NOTICE file distributed with
 * this work for additional information regarding copyright ownership.
 * The ASF licenses this file to You under the Apache License, Version 2.0
 * (the "License"); you may not use this file except in compliance with
 * the License.  You may obtain a copy of the License at
 *
 *     http://www.apache.org/licenses/LICENSE-2.0
 *
 * Unless required by applicable law or agreed to in writing, software
 * distributed under the License is distributed on an "AS IS" BASIS,
 * WITHOUT WARRANTIES OR CONDITIONS OF ANY KIND, either express or implied.
 * See the License for the specific language governing permissions and
 * limitations under the License.
 */

#include "apr_arch_file_io.h"
#include "apr_strings.h"
#include "apr_portable.h"

#ifdef __VMS
#include "protshr.h"
#ifndef BG_PIPE_BUFFER_SIZE_DEFAULT
#define BG_PIPE_BUFFER_SIZE_DEFAULT 32767
#endif
#ifndef MB_PIPE_BUFFER_SIZE_DEFAULT
#define MB_PIPE_BUFFER_SIZE_DEFAULT 4096
#endif
extern int decc$$translate (int);
#endif

#include "apr_arch_inherit.h"

/* Figure out how to get pipe block/nonblock on BeOS...
 * Basically, BONE7 changed things again so that ioctl didn't work,
 * but now fcntl does, hence we need to do this extra checking.
 * The joys of beta programs. :-)
 */
#if defined(BEOS)
#if !defined(BONE7)
# define BEOS_BLOCKING 1
#else
# define BEOS_BLOCKING 0
#endif
#endif

static apr_status_t pipeblock(apr_file_t *thepipe)
{
#if defined(__VMS)
   int off = 0;
   if (thepipe->is_socket)
      if (ioctl (thepipe->filedes, FIONBIO, &off) < 0)
         return errno;

#elif !defined(BEOS) || !BEOS_BLOCKING
      int fd_flags;

      fd_flags = fcntl(thepipe->filedes, F_GETFL, 0);
#  if defined(O_NONBLOCK)
      fd_flags &= ~O_NONBLOCK;
#  elif defined(O_NDELAY)
      fd_flags &= ~O_NDELAY;
#  elif defined(O_FNDELAY)
      fd_flags &= ~O_FNDELAY;
#  else
      /* XXXX: this breaks things, but an alternative isn't obvious...*/
      return APR_ENOTIMPL;
#  endif
      if (fcntl(thepipe->filedes, F_SETFL, fd_flags) == -1) {
          return errno;
      }
#else /* BEOS_BLOCKING */

#  if BEOS_BONE /* This only works on BONE 0-6 */
      int on = 0;
      if (ioctl(thepipe->filedes, FIONBIO, &on, sizeof(on)) < 0) {
          return errno;
      }
#  else /* "classic" BeOS doesn't support this at all */
      return APR_ENOTIMPL;
#  endif

#endif /* !BEOS_BLOCKING */

    thepipe->blocking = BLK_ON;
    return APR_SUCCESS;
}

static apr_status_t pipenonblock(apr_file_t *thepipe)
{
#if defined(__VMS)
   int on = 1;
   if (thepipe->is_socket)
      if (ioctl (thepipe->filedes, FIONBIO, &on) < 0)
         return errno;

#elif !defined(BEOS) || !BEOS_BLOCKING
      int fd_flags = fcntl(thepipe->filedes, F_GETFL, 0);

#  if defined(O_NONBLOCK)
      fd_flags |= O_NONBLOCK;
#  elif defined(O_NDELAY)
      fd_flags |= O_NDELAY;
#  elif defined(O_FNDELAY)
      fd_flags |= O_FNDELAY;
#  else
      /* XXXX: this breaks things, but an alternative isn't obvious...*/
      return APR_ENOTIMPL;
#  endif
      if (fcntl(thepipe->filedes, F_SETFL, fd_flags) == -1) {
          return errno;
      }

#else /* BEOS_BLOCKING */

#  if BEOS_BONE /* This only works on BONE 0-6 */
      int on = 1;
      if (ioctl(thepipe->filedes, FIONBIO, &on, sizeof(on)) < 0) {
          return errno;
      }
#  else /* "classic" BeOS doesn't support this at all */
      return APR_ENOTIMPL;
#  endif

#endif /* !BEOS_BLOCKING */

    thepipe->blocking = BLK_OFF;
    return APR_SUCCESS;
}

APR_DECLARE(apr_status_t) apr_file_pipe_timeout_set(apr_file_t *thepipe, apr_interval_time_t timeout)
{
    if (thepipe->is_pipe == 1) {
        thepipe->timeout = timeout;
        if (timeout >= 0) {
            if (thepipe->blocking != BLK_OFF) { /* blocking or unknown state */
                return pipenonblock(thepipe);
            }
        }
        else {
            if (thepipe->blocking != BLK_ON) { /* non-blocking or unknown state */
                return pipeblock(thepipe);
            }
        }
        return APR_SUCCESS;
    }
    return APR_EINVAL;
}

APR_DECLARE(apr_status_t) apr_file_pipe_timeout_get(apr_file_t *thepipe, apr_interval_time_t *timeout)
{
    if (thepipe->is_pipe == 1) {
        *timeout = thepipe->timeout;
        return APR_SUCCESS;
    }
    return APR_EINVAL;
}

APR_DECLARE(apr_status_t) apr_os_pipe_put_ex(apr_file_t **file,
                                             apr_os_file_t *thefile,
                                             int register_cleanup,
                                             apr_pool_t *pool)
{
    int *dafile = thefile;

    (*file) = apr_pcalloc(pool, sizeof(apr_file_t));
    (*file)->pool = pool;
    (*file)->eof_hit = 0;
    (*file)->is_pipe = 1;
    (*file)->blocking = BLK_UNKNOWN; /* app needs to make a timeout call */
    (*file)->timeout = -1;
    (*file)->ungetchar = -1; /* no char avail */
    (*file)->filedes = *dafile;
    if (!register_cleanup) {
        (*file)->flags = APR_FOPEN_NOCLEANUP;
    }
    (*file)->buffered = 0;
#if APR_HAS_THREADS
    (*file)->thlock = NULL;
#endif
    if (register_cleanup) {
        apr_pool_cleanup_register((*file)->pool, (void *)(*file),
                                  apr_unix_file_cleanup,
                                  apr_pool_cleanup_null);
    }
#ifndef WAITIO_USES_POLL
    /* Start out with no pollset.  apr_wait_for_io_or_timeout() will
     * initialize the pollset if needed.
     */
    (*file)->pollset = NULL;
#endif
    return APR_SUCCESS;
}

APR_DECLARE(apr_status_t) apr_os_pipe_put(apr_file_t **file,
                                          apr_os_file_t *thefile,
                                          apr_pool_t *pool)
{
    return apr_os_pipe_put_ex(file, thefile, 0, pool);
}

#ifdef __VMS
static apr_status_t file_pipe_create(apr_file_t **in, apr_file_t **out,
        apr_pool_t *pool_in, apr_pool_t *pool_out, int UseSocketPipes)
#else
static apr_status_t file_pipe_create(apr_file_t **in, apr_file_t **out,
        apr_pool_t *pool_in, apr_pool_t *pool_out)
#endif
{
#ifdef __VMS
    static int BgPipeBufferSize = -1;
    static int MbPipeBufferSize = -1;
    char *PipeBufferSizePtr = NULL;
    int status;
#endif
    int filedes[2];

#ifdef __VMS
    if (UseSocketPipes) {
        if (BgPipeBufferSize < 0) {
            if ((PipeBufferSizePtr = getenv("APACHE$BG_PIPE_BUFFER_SIZE")) == NULL) {
               PipeBufferSizePtr = getenv("APR$BG_PIPE_BUFFER_SIZE");
            }
            if (PipeBufferSizePtr)
                BgPipeBufferSize = atoi(PipeBufferSizePtr);
            else
                BgPipeBufferSize = BG_PIPE_BUFFER_SIZE_DEFAULT;
        }

        status = socketpair(AF_INET, SOCK_STREAM, 0, filedes);
        if (status == 0) {
            status =
                apr$$setsockopt(vaxc$get_sdc(filedes[0]),
                                   SET_SOCK_DEV_BUF, &BgPipeBufferSize,
                                   sizeof(BgPipeBufferSize));
            if (!(status & 1)) {
                decc$$translate(status);
                status = -1;
            } else {
                status = 0;
            }
        }
        if (status == 0) {
            status =
                apr$$setsockopt(vaxc$get_sdc(filedes[1]),
                                   SET_SOCK_DEV_BUF, &BgPipeBufferSize,
                                   sizeof(BgPipeBufferSize));
            if (!(status & 1)) {
                decc$$translate(status);
                status = -1;
            } else {
                status = 0;
            }
        }
    } else {
        if (MbPipeBufferSize < 0) {
            if ((PipeBufferSizePtr = getenv("APACHE$MB_PIPE_BUFFER_SIZE")) == NULL) {
               PipeBufferSizePtr = getenv("APR$MB_PIPE_BUFFER_SIZE");
            }
            if (!PipeBufferSizePtr)
                PipeBufferSizePtr = getenv("DECC$PIPE_BUFFER_SIZE");
            if (PipeBufferSizePtr)
                MbPipeBufferSize = atoi(PipeBufferSizePtr);
            else
                MbPipeBufferSize = MB_PIPE_BUFFER_SIZE_DEFAULT;
        }

        status = pipe(filedes, 0, MbPipeBufferSize);
    }

    if (status == -1) {
        return errno;
    }
#else
    if (pipe(filedes) == -1) {
        return errno;
    }
#endif

    (*in) = (apr_file_t *)apr_pcalloc(pool_in, sizeof(apr_file_t));
    (*in)->pool = pool_in;
    (*in)->filedes = filedes[0];
    (*in)->is_pipe = 1;
    (*in)->fname = NULL;
    (*in)->buffered = 0;
    (*in)->blocking = BLK_ON;
    (*in)->timeout = -1;
    (*in)->ungetchar = -1;
    (*in)->flags = APR_INHERIT;
#if APR_HAS_THREADS
    (*in)->thlock = NULL;
#endif
#ifndef WAITIO_USES_POLL
    (*in)->pollset = NULL;
#endif
#ifdef __VMS
    (*in)->is_socket = UseSocketPipes ? TRUE : FALSE;
    (*in)->skipped_first_pipe_eof = FALSE;
    (*in)->pipe_buffer_size = UseSocketPipes ? BgPipeBufferSize : MbPipeBufferSize;
#endif

    (*out) = (apr_file_t *)apr_pcalloc(pool_out, sizeof(apr_file_t));
    (*out)->pool = pool_out;
    (*out)->filedes = filedes[1];
    (*out)->is_pipe = 1;
    (*out)->fname = NULL;
    (*out)->buffered = 0;
    (*out)->blocking = BLK_ON;
    (*out)->flags = APR_INHERIT;
    (*out)->timeout = -1;
#if APR_HAS_THREADS
    (*out)->thlock = NULL;
#endif
#ifndef WAITIO_USES_POLL
    (*out)->pollset = NULL;
#endif
#ifdef __VMS
    (*out)->is_socket = UseSocketPipes ? TRUE : FALSE;
    (*out)->skipped_first_pipe_eof = FALSE;
    (*out)->pipe_buffer_size = UseSocketPipes ? BgPipeBufferSize : MbPipeBufferSize;
#endif

    apr_pool_cleanup_register((*in)->pool, (void *)(*in), apr_unix_file_cleanup,
                         apr_pool_cleanup_null);
    apr_pool_cleanup_register((*out)->pool, (void *)(*out), apr_unix_file_cleanup,
                         apr_pool_cleanup_null);
    return APR_SUCCESS;
}

static void file_pipe_block(apr_file_t **in, apr_file_t **out, apr_int32_t blocking)
{
    switch (blocking) {
    case APR_FULL_BLOCK:
        break;
    case APR_READ_BLOCK:
        apr_file_pipe_timeout_set(*out, 0);
        break;
    case APR_WRITE_BLOCK:
        apr_file_pipe_timeout_set(*in, 0);
        break;
    default:
        apr_file_pipe_timeout_set(*out, 0);
        apr_file_pipe_timeout_set(*in, 0);
        break;
    }
}

#ifdef __VMS
APR_DECLARE(apr_status_t) apr_file_pipe_create(apr_file_t **in,
        apr_file_t **out, apr_pool_t *pool, ...)
#else
APR_DECLARE(apr_status_t) apr_file_pipe_create(apr_file_t **in,
        apr_file_t **out, apr_pool_t *pool)
#endif
{
#ifdef __VMS
    int UseSocketPipes = FALSE;
    va_list argp;
    int argc;

    va_start(argp, pool);
    va_count(argc);
    if (argc > 3)
        UseSocketPipes = va_arg(argp, int);
    va_end(argp);

    return file_pipe_create(in, out, pool, pool, UseSocketPipes);
#else
    return file_pipe_create(in, out, pool, pool);
#endif
}

#ifdef __VMS
APR_DECLARE(apr_status_t) apr_file_pipe_create_ex(apr_file_t **in,
                                                  apr_file_t **out,
                                                  apr_int32_t blocking,
                                                  apr_pool_t *pool, ...)
#else
APR_DECLARE(apr_status_t) apr_file_pipe_create_ex(apr_file_t **in,
                                                  apr_file_t **out,
                                                  apr_int32_t blocking,
                                                  apr_pool_t *pool)
#endif
{
    apr_status_t status;
#ifdef __VMS
    int UseSocketPipes = FALSE;
    va_list argp;
    int argc;

    va_start(argp, pool);
    va_count(argc);
    if (argc > 4)
        UseSocketPipes = va_arg(argp, int);
    va_end(argp);

    if ((status = file_pipe_create(in, out, pool, pool, UseSocketPipes)) != APR_SUCCESS) {
#else
    if ((status = file_pipe_create(in, out, pool, pool)) != APR_SUCCESS) {
#endif
        return status;
    }

    file_pipe_block(in, out, blocking);

    return APR_SUCCESS;
}

#ifdef __VMS
APR_DECLARE(apr_status_t) apr_file_pipe_create_pools(apr_file_t **in,
        apr_file_t **out, apr_int32_t blocking, apr_pool_t *pool_in, apr_pool_t *pool_out, ...)
#else
APR_DECLARE(apr_status_t) apr_file_pipe_create_pools(apr_file_t **in,
        apr_file_t **out, apr_int32_t blocking, apr_pool_t *pool_in, apr_pool_t *pool_out)
#endif
{
    apr_status_t status;
#ifdef __VMS
    int UseSocketPipes = FALSE;
    va_list argp;
    int argc;

    va_start(argp, pool_out);
    va_count(argc);
    if (argc > 5)
        UseSocketPipes = va_arg(argp, int);
    va_end(argp);

    if ((status = file_pipe_create(in, out, pool_in, pool_out, UseSocketPipes)) != APR_SUCCESS) {
#else
    if ((status = file_pipe_create(in, out, pool_in, pool_out)) != APR_SUCCESS) {
#endif
        return status;
    }

    file_pipe_block(in, out, blocking);

    return APR_SUCCESS;
}

APR_DECLARE(apr_status_t) apr_file_namedpipe_create(const char *filename,
                                                    apr_fileperms_t perm, apr_pool_t *pool)
{
#ifdef __VMS
    errno = ENOTSUP;
    return (errno);
#else
    mode_t mode = apr_unix_perms2mode(perm);

    if (mkfifo(filename, mode) == -1) {
        return errno;
    }
    return APR_SUCCESS;
#endif
}



