...
extern int apr$master_pid (void);
extern int apr$owner_pid (void);
extern int apr$parent_pid (void);

extern decc$$translate ();
                                            ...
                                            const char *fname)
{
    ...
    if (fname) {
        ...
        char *DefDir = "";
        apr_uid_homepath_get (&DefDir, getlogin (), new_mutex->pool);
        new_mutex->fname = apr_pcalloc (new_mutex->pool, strlen (DefDir) + strlen ("/aprXXXXXXXX.tmp"));
        sprintf (new_mutex->fname, "%s/apr%08X.tmp", DefDir, apr$parent_pid ());
        rv = apr_file_open (&new_mutex->interproc, new_mutex->fname,
                            APR_CREATE | APR_WRITE | APR_DELONCLOSE,
                            APR_UREAD | APR_UWRITE,
                            new_mutex->pool);
    }
#else
...
#ifdef __VMS
#if APR_HAS_VMSDLM_SERIALIZE

static void proc_mutex_vmsdlm_setup(void);
static apr_status_t proc_mutex_vmsdlm_cleanup(void *);
static apr_status_t proc_mutex_vmsdlm_create(apr_proc_mutex_t *,
                                             const char *);
static apr_status_t proc_mutex_vmsdlm_acquire(apr_proc_mutex_t *);
static apr_status_t proc_mutex_vmsdlm_release(apr_proc_mutex_t *);
static apr_status_t proc_mutex_vmsdlm_destroy(void *);

static void proc_mutex_vmsdlm_setup(void)
{
}

static apr_status_t proc_mutex_vmsdlm_cleanup(void *mutex)
{
    unsigned int lockid;
    int status;

/*
** Make sure the mutex is valid
*/
    if (!mutex) {
...
/*
** If the mutex is locked, then release it now
*/
    lockid = ((apr_proc_mutex_t *) mutex)->lockid;
    if (lockid) {
        /*
         ** Cancel any pending lock
         */
        SYS$DEQ(lockid,         /* Lock ID                      */
                0,              /* No value block               */
        ...
        ((apr_proc_mutex_t *) mutex)->curr_locked = 0;
        ((apr_proc_mutex_t *) mutex)->lockid = 0;
    }
...
static char *mkname(char *val)
{
        static int incr = 0;

        incr++;
        sprintf(val, "APACHE$%08lX_%08d", getpid(), incr);
        return (val);
}

static apr_status_t proc_mutex_vmsdlm_create(apr_proc_mutex_t * new_mutex,
                                             const char *fname)
{
    struct dsc$descriptor MutexDesc =
        { 0, DSC$K_DTYPE_T, DSC$K_CLASS_S, 0 };
    LKSB MutexLKSB;
    int status;

/*
** Make sure the mutex & filename are valid
*/
    if (!new_mutex) {
        decc$$translate(SS$_BADPARAM);
        return errno;
...
*/
    if (fname) {
        ...
        char val[ID_LEN + 1];
        new_mutex->fname = apr_pstrdup(new_mutex->pool, mkname(val));
    }

/*
** Create the lock resouce name descriptor
*/
    MutexDesc.dsc$w_length = strlen(new_mutex->fname);
    MutexDesc.dsc$a_pointer = new_mutex->fname;
...
** Enqueue the lock
*/
    status = SYS$ENQW(EFN$C_ENF,        /* Lock event flag              */
    ...
    if (status == SS$_NORMAL)
        status = MutexLKSB.lksb$w_status;
    if (status != SS$_NORMAL) {
        decc$$translate(status);
        return errno;
    }
...
*/
    new_mutex->curr_locked = 0;
    new_mutex->lockid = MutexLKSB.lksb$l_lkid;

/*
** Register the cleanup for this lock
*/
    apr_pool_cleanup_register(new_mutex->pool, (void *) new_mutex,
                              proc_mutex_vmsdlm_cleanup,
                              apr_pool_cleanup_null);
    return APR_SUCCESS;
}

static apr_status_t proc_mutex_vmsdlm_acquire(apr_proc_mutex_t * mutex)
{
    LKSB MutexLKSB;
    int status;

/*
** Make sure the mutex & lockid are valid
*/
    if (!mutex || !mutex->lockid) {
        decc$$translate(SS$_BADPARAM);
        return errno;
    }
...
*/
    memset(&MutexLKSB, 0, sizeof(MutexLKSB));
    MutexLKSB.lksb$l_lkid = mutex->lockid;
...
*/
    status = SYS$ENQW(EFN$C_ENF,        /* Lock event flag              */
                      ...
                      &MutexLKSB,       /* Lock Status Block            */
                      LCK$M_CONVERT,    /* No lock flags                */
                      0,                /* Resource name descriptor     */
                      0,                /* No parent                    */
                      0,                /* No AST routine               */
                      0,                /* No AST parameter             */
                      0,                /* No blocking AST              */
...
static apr_status_t proc_mutex_vmsdlm_tryacquire(apr_proc_mutex_t * mutex)
{
    LKSB MutexLKSB;
    int status;
...
static apr_status_t proc_mutex_vmsdlm_timedacquire(apr_proc_mutex_t *mutex,
                                                  apr_interval_time_t timeout)
{
#define SLEEP_TIME apr_time_from_msec(10)
    apr_status_t rv;
...
static apr_status_t proc_mutex_vmsdlm_release(apr_proc_mutex_t * mutex)
{
    LKSB MutexLKSB;
    int status;
...
static apr_status_t proc_mutex_vmsdlm_destroy(void *mutex_ptr)
{
    apr_proc_mutex_t *mutex = (apr_proc_mutex_t *) mutex_ptr;
    apr_status_t rv;

    if ((rv = proc_mutex_vmsdlm_cleanup(mutex)) == APR_SUCCESS) {
        apr_pool_cleanup_kill(mutex->pool, mutex,
                              proc_mutex_vmsdlm_cleanup);
        return APR_SUCCESS;
    }
...
static apr_status_t proc_mutex_vmsdlm_child_init(apr_proc_mutex_t ** mutex,
                                                 apr_pool_t * pool,
                                                 const char *fname)
{
    apr_proc_mutex_t *new_mutex;
    ...
    if (mutex == NULL) {
        printf("proc_mutex_vmsdlm_child_init: Mutex is null\n");
        return APR_SUCCESS;
    }
    if (pool == NULL) {
        printf("proc_mutex_vmsdlm_child_init: Pool is null\n");
        return APR_SUCCESS;
    }
    if (fname == NULL) {
/*      printf("proc_mutex_vmsdlm_child_init: Fname is null\n");        */
        return APR_SUCCESS;
    }

    printf("proc_mutex_vmsdlm_child_init: Fname is %s\n", fname);

    new_mutex =
        (apr_proc_mutex_t *) apr_palloc(pool, sizeof(apr_proc_mutex_t));
...
static const apr_proc_mutex_unix_lock_methods_t mutex_vmsdlm_methods = {
    APR_PROCESS_LOCK_MECH_IS_GLOBAL,
    proc_mutex_vmsdlm_create,
    proc_mutex_vmsdlm_acquire,
    proc_mutex_vmsdlm_tryacquire,
    proc_mutex_vmsdlm_timedacquire,
    proc_mutex_vmsdlm_release,
    proc_mutex_vmsdlm_destroy,
    proc_mutex_vmsdlm_child_init,
    proc_mutex_no_perms_set,            /* BRC */
    APR_LOCK_VMSDLM,                    /* BRC */
    "vmsdlm"
};

#endif                          /* APR_HAS_VMSDLM_SERIALIZE */
#endif                          /* __VMS */
...
#endif
#if APR_HAS_SYSVSEM_SERIALIZE || APR_HAS_FCNTL_SERIALIZE || APR_HAS_FLOCK_SERIALIZE || APR_HAS_VMSDLM_SERIALIZE  /* __VMS */
    new_mutex->os.crossproc = -1;
...
#elif APR_USE_VMSDLM_SERIALIZE && defined(__VMS)
        new_mutex->meth = &mutex_vmsdlm_methods;
        if (ospmutex) {
            if (ospmutex->crossproc == -1) {
                return APR_EINVAL;
            }
