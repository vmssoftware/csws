...
#ifndef BEOS_R5
#ifdef __VMS
    status = apr$$socket (family, type, 0, &SockChan);
    if (status & 1)
        (*new)->socketdes = decc$socket_fd (SockChan);
    else
#endif
...
#if APR_HAVE_IPV6
#ifdef __VMS
    if (apr$ipv6_started () && (*new)->socketdes < 0 && ofamily == APR_UNSPEC) {
#else
...
#if APR_HAVE_IPV6
#ifdef __VMS
    else if (apr$ipv6_started () && sock->local_addr->sa.sin.sin_family == AF_INET6) {
#else
