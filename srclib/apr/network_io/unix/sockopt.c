...
#ifdef __VMS
                                apr_int64_t opt, apr_int32_t on)
#else
...
#endif
{
    ...
    case APR_VMS_SO_CCL:
        {
        int status;
        status = apr$$setsockopt (vaxc$get_sdc (sock->socketdes), SET_SOCK_DEV_CCL, &on, sizeof (on));
        if (! (status & 1))
            {
            decc$$translate (status);
            return (errno);
            }
        apr_set_option (sock, APR_VMS_SO_CCL, on);
        break;
        }
    ...
    case APR_VMS_SO_SHR:
        {
        int status;
        status = apr$$setsockopt (vaxc$get_sdc (sock->socketdes), SET_SOCK_DEV_SHR, &on, sizeof (on));
        if (! (status & 1))
            {
            decc$$translate (status);
            return (errno);
            }
        apr_set_option (sock, APR_VMS_SO_SHR, on);
        break;
        }
#endif          /* __VMS */
