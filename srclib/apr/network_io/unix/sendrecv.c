...
#ifdef __VMS
#ifndef IOV_TIMEOUT_DEFAULT
#define IOV_TIMEOUT_DEFAULT 15
#endif
...
static ssize_t BRC_writev(int fildes, const struct iovec * iov, int iovcnt)
{
    struct timeval tv, *tvp;
    int is_sock = 0, iovlen = 0, rv, i, j;
    char *iovptr;
    fd_set fdset;
    char *tmp;
    ...
    if (decc$get_sdc(fildes)) {
        is_sock = 1;
    }

    for (i = 0; i < iovcnt; i++) {
        for (j = iov[i].iov_len, iovptr = iov[i].iov_base; j > 0;) {
            if (is_sock) {
                if (timeout < 0) {
                    if ((tmp = getenv("APACHE_IOV_TIMEOUT"))) {
                        timeout = atoi(tmp);
                    } else {
                        timeout = IOV_TIMEOUT_DEFAULT;
                    }
                }
                ...
                do {
                    ...
                    rv = select(fildes + 1, NULL, &fdset, NULL, tvp);
                }
                while (rv == -1 && errno == EINTR);
                ...
                rv = send(fildes, iovptr, MIN(j, BUFSIZ), 0);
            } else {
                rv = write(fildes, iovptr, MIN(j, BUFSIZ));
            }

            if (rv < 0 || (rv == 0 && is_sock && timeout)) {
                return (-1);
            }
