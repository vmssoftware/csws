...
#ifdef __VMS
#include "apr_arch_file_io.h"   /* define apr_file_t structure */
#endif
                                  ...
                                  cgi_exec_info_t *e_info)
{
...
#ifdef __VMS
    static int UseSocketPipes = TRUE;
#endif
...
#else
#ifdef __VMS
    FILE *dbg = stderr;         /* Output will end up in the error log */
#else
     ...
     */
#ifdef __VMS
    if (((rc = apr_procattr_create(&procattr, p, r->handler, UseSocketPipes)) != APR_SUCCESS) ||
#else
...
static int cgi_handler(request_rec *r)
{
    ...
    if (r->finfo.filetype == 0)
        {
        static char *FileExtensionList[] = {".COM",".EXE",NULL};
        apr_finfo_t FileInfo;
        char *TestFile;
        int i;

        /*
        ** Look through the file extension list to determine whether the file
        ** exists with one them
        */
        for (i = 0; FileExtensionList[i]; i++)
            {
            TestFile = apr_pstrcat (p, r->filename, FileExtensionList[i], NULL);
            if (apr_stat (&FileInfo, TestFile, APR_FINFO_MIN, p) == APR_SUCCESS)
                {
                r->filename = TestFile;
                r->finfo = FileInfo;
                break;
