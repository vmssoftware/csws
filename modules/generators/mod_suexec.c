                                   ...
                                   const char *uid, const char *gid)
{
    ...
    IdentDesc.dsc$w_length = strlen(IdentDesc.dsc$a_pointer);
    ...
    status =
        apr$$check_rights_list(ap_unixd_config.user_name,
                                  strlen(ap_unixd_config.user_name), &Ident,
                                  1);
    if (!(status & 1)) {
        fprintf(stderr,
                "Warning: Server Account %s is not authorized to run suEXEC.\n",
    ...
    status = apr$$check_rights_list(uid, strlen(uid), &Ident, 1);
    if (!(status & 1)) {
        fprintf(stderr,
                "Warning: User Account %s is not authorized to be run via suEXEC.\n",
                uid);
        return NULL;
    }
#endif
