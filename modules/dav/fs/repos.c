...
#ifdef __VMS
#include <unixlib.h>            /* File conversion routine */
#include <fabdef.h>
static int vms_fixup_file (char *fn, int fl);
static apr_status_t vms_check_parent(const char *path, apr_pool_t *p);
#endif
    ...
    dav_buffer *pbuf)
{
...
#ifdef __VMS
    if ((status = apr_file_open(&inf, src, APR_READ | APR_BINARY | APR_CTX_STM,
    ...
    status = apr_file_open(&outf, dst, APR_WRITE | APR_CREATE | APR_TRUNCATE
                           | APR_BINARY | APR_CTX_STM, perms, p);
#else
    status = apr_file_open(&outf, dst, APR_WRITE | APR_CREATE | APR_TRUNCATE
                           | APR_BINARY, perms, p);
#endif
    ...
    dav_buffer *pbuf)
{
    ...
    strcpy(tmp, dst);
    strcat(tmp, ".DIR");
    if ((rv = chmod(tmp, (S_IRWXU | S_IRGRP))) != 0) {
        (void) apr_dir_remove(dst, p);
        return dav_new_error(p, HTTP_INTERNAL_SERVER_ERROR, 0, rv,
                             "Could not set protection on state directory");
    }
#endif
...
static apr_status_t dav_fs_mktemp(apr_file_t **fp, char *templ, apr_pool_t *p)
{
    ...
    do {
        ...
        rv = apr_file_open(fp, templ,
                           APR_WRITE | APR_CREATE | APR_BINARY | APR_EXCL | APR_CTX_STM,
                           APR_OS_DEFAULT, p);
#else
        rv = apr_file_open(fp, templ,
                                  ...
                                  ap_filter_t *output)
{
    ...
    if ((status = apr_file_open(&fd, resource->info->pathname,
                                APR_READ | APR_BINARY |
                                (((resource->info->finfo.fab_rfm == FAB$C_VAR) ||
                                 (resource->info->finfo.fab_rfm == FAB$C_VFC)) ?
                                 APR_CTX_REC : APR_CTX_STM), 0,
                                pool)) != APR_SUCCESS) {
#else
    if ((status = apr_file_open(&fd, resource->info->pathname,
...
#ifdef __VMS
    apr_status_t vms_check_parent(const char *path, apr_pool_t *p);

    /*
    ** VMS, by default, will create the parent directory
    ** if it does not exist.  This does not match what happens
    ** on UNIX so we are removing that feature of VMS so that
    ** WebDAV operates exactly the same on both platforms.
    */
    status = vms_check_parent(ctx->pathname, ctx->pool);
    if (APR_STATUS_IS_ENOENT(status)) {
...
#ifdef __VMS
    /*
    ** Make sure protection is set so that the owner can
    ** change the name of the collection.
    */
    int sz = strlen(ctx->pathname) + 5;
    char *tmp;
    tmp = apr_palloc(ctx->pool, sz);

    strcpy(tmp, ctx->pathname);
    strcat(tmp, ".DIR");
    if ((status = chmod(tmp, (S_IRWXU | S_IRGRP))) != 0) {
        (void) apr_dir_remove(ctx->pathname, ctx->pool);
        return dav_new_error(ctx->pool, HTTP_CONFLICT, 0, status,
                             "Cannot set collection protection; error "
                             "from chmod() function.");
    }
#endif
...
#ifdef __VMS
            /*
            ** Set protection on new collection so that it
            ** can be renamed by the owner.
            */

            apr_status_t status;
            int sz = strlen(dstinfo->pathname) + 5;
            char *tmp;
            tmp = apr_palloc(ctx->pool, sz);

            strcpy(tmp, dstinfo->pathname);
            strcat(tmp, ".DIR");
            if ((status = chmod(tmp, (S_IRWXU | S_IRGRP))) != 0) {
                err = dav_new_error(ctx->pool, HTTP_FORBIDDEN, 0, status,
                                    "Could not set protection on collection.");
            }
#endif
        }
    }
...
#ifdef __VMS
#define VMS_MAX_FILEBUF  256
static char savfile[VMS_MAX_FILEBUF];
#endif
...
#ifdef __VMS
    /*
    ** rename() is picky about filespecs.  Convert them to
    ** VMS format and continue with the rename.  If the
    ** rename fails, it might be because the rename attempt
    ** is for a directory.  VMS directories all end with ".DIR".
    ** So make a 2nd attempt.  Note that the path buffers have
    ** extra room just for this purpose.
    */
    n = decc$to_vms(srcinfo->pathname, vms_fixup_file, 0, 0);
    srcpath = apr_palloc(srcinfo->pool, strlen(savfile)+5);
    strcpy(srcpath, savfile);

    n = decc$to_vms(dstinfo->pathname, vms_fixup_file, 0, 0);
    dstpath = apr_palloc(dstinfo->pool, strlen(savfile)+5);
    strcpy(dstpath, savfile);
    if ((rv = apr_file_rename(srcpath, dstpath, srcinfo->pool)) != APR_SUCCESS) {
        strcat(srcpath, ".DIR");
        strcat(dstpath, ".DIR");
        rv = apr_file_rename(srcpath, dstpath, srcinfo->pool);
    }
#else
...
#ifdef __VMS
    if (apr_file_rename(dstpath, srcpath, srcinfo->pool) != APR_SUCCESS) {
#else
...
static apr_status_t vms_check_parent(const char *path, apr_pool_t *p)
{
    ...
    *c = '\0';
    ...
    return apr_stat(&info, parent, APR_FINFO_TYPE, p);
}

static int vms_fixup_file (char *fn, int fl)
{
    char *find_last_dot(char *);

    int sz;
    char *c;
    char *pc;
    char tmp[VMS_MAX_FILEBUF];

    strcpy(savfile, fn);

    sz = strlen(savfile);
    if (savfile[sz-1] == ']') {
        c = find_last_dot(savfile);
        if (c == NULL) {
            c = strchr(savfile, '[');
            c++;
            strcpy(tmp, savfile);
