...
#ifdef __VMS
    /*
    ** Check to see if there is a specific DBM to be used
    */
    if (DavDbmType == (char *) -1)
        DavDbmType = getenv ("APACHE$DAV_DBM_TYPE");

    if (DavDbmType)
        {
        status = apr_dbm_open_ex (&file, DavDbmType, pathname,
                                  ro ? APR_DBM_READONLY : APR_DBM_RWCREATE,
                                  APR_OS_DEFAULT, p);
        if (status != APR_SUCCESS && !ro)
