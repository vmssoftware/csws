...
#ifdef __VMS
    stat = apr_socket_opt_set(s, APR_VMS_SO_SHR, one);
    if (stat != APR_SUCCESS && stat != APR_ENOTIMPL) {
        ap_log_perror(APLOG_MARK, APLOG_CRIT, stat, p,
                      "make_sock: for address %pI, apr_socket_opt_set: (VMS_SO_SHR)",
                      server->bind_addr);
        apache$sps_socket_close(s);
        return stat;
    }
#endif
...
#ifdef __VMS
    if ((stat = apache$sps_socket_bind(s, server->bind_addr)) != APR_SUCCESS) {
#else
                                  ...
                                  void *slave)
{
    ...
    while (sa) {
        ...
        status = apache$sps_socket_create (&new->sd,
                                            new->bind_addr->family,
                                            SOCK_STREAM, process->pool, ips);
#else
...
static int open_listeners(apr_pool_t *pool)
{
    ...
    for (lr = ap_listeners; lr; previous = lr, lr = lr->next) {
        if (lr->active) {
...
#ifdef __VMS
            /* Lock access to this socket port */
            switch(default_family()) {
               case APR_INET:
                  if ((lr->bind_addr->hostname) && (strcmp(lr->bind_addr->hostname, "0.0.0.0") != 0)) {
                     sprintf(ips, "%s:%d",   lr->bind_addr->hostname, lr->bind_addr->port);
                  } else {
                     sprintf(ips, "%d", lr->bind_addr->port);
                  }
                  break;
#if APR_HAVE_IPV6
               case APR_INET6:
                  if ((lr->bind_addr->hostname) && (strcmp(lr->bind_addr->hostname, "::") != 0)) {
                     sprintf(ips, "[%s]:%d",  lr->bind_addr->hostname, lr->bind_addr->port);
                  } else {
                     sprintf(ips, "%s:%d", lr->bind_addr->hostname, lr->bind_addr->port);
                  }
            ...
            status = apache$sps_port_lock (ips);
            if (status != APR_SUCCESS)
                fprintf (stderr, "WARNING (%d,%d,0x%08X): Unable to lock shared socket device for %s\n",
                         status, errno, vaxc$errno, ips);

            /*
            ** Make the socket
            */
            make_status = make_sock (pool, lr);

            /*
            ** Lock access to this socket port
            */
            status = apache$sps_port_unlock (ips);
            if (status != APR_SUCCESS)
                fprintf (stderr, "WARNING (%d,%d,0x%08X): Unable to unlock shared socket device for %s (%s)\n",
                         status, errno, vaxc$errno, ips, "open_listeners()");

            /*
            ** Test the make_sock status
            */
            if (make_status == APR_SUCCESS) {
#else
#if APR_HAVE_IPV6
            ap_listen_rec *cur;
            int v6only_setting;
                                                ...
                                                int argc, char *const argv[])
{
    ...
    if(!host) {
        ...
        {
           case APR_INET:
              sprintf(newips, "%d", port);
              break;
#if APR_HAVE_IPV6
           case APR_INET6:
              sprintf(newips,"%s:%d","::",port);
              break;
#endif
        }
    }
    else {
       sprintf(newips,"%s", argv[0]);
    }
    /*
    ** Lock access to this socket port
    */
    status = apache$sps_port_lock (newips);
    if (status != APR_SUCCESS)
        fprintf (stderr, "WARNING (%d,%d,0x%08X): Unable to lock shared socket device for %s\n",
                         status, errno, vaxc$errno,newips);

    /*
    ** Allocate the listener
    */
    alloc_status = alloc_listener (cmd->server->process, host, port, proto, NULL);

    /*
    ** Unlock access to this socket port
    */
    status = apache$sps_port_unlock (newips);
    if (status != APR_SUCCESS)
        fprintf (stderr, "WARNING (%d,%d,0x%08X): Unable to unlock shared socket device for %s (%s)\n",
                 status, errno, vaxc$errno, newips, "ap_set_listener()");

    /*
    ** Save the listen port in the server record
    */
    cmd->server->port = cmd->server->port ? cmd->server->port : port;

    /*
    ** Return the alloc_listener status
    */
    return (alloc_status);
#else
