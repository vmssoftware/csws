...
{
#ifdef __VMS
    int VMScgiblankcheck=1; /* RMS puts a blank line before the header
                                   ignore the first blank line
                                   However, if in binary mode, we don't
                                   get a blank line.  Powell "Hap" Hazzard*/
#endif
    ...
    while (1) {
        ...
        if (rv == 0) {
#ifdef __VMS    // BRC - need to revisit this
        ...
        if (VMScgiblankcheck)
            if (w[0] == '\0')
                {
                /*
                ** only do this for cgi-script or it will break osu-script...
                */
                if (!strcmp ("cgi-script", r->handler))
                    {
                    VMScgiblankcheck = 0;
                    continue; /* go back to the while */
                    }
                }
        VMScgiblankcheck = 0; /* pass the blank line check */

        if ((w[0] == '\015') && (w[1] == '\0'))         /* cr treat as a blank line */
                w[0] = '\0';                            /* write sys$output "" needs to be treated as the end of the header */
#endif
