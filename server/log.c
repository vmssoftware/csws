                                               ...
                                               const char *fname)
{
...
#ifdef __VMS
    if ((rc = apache$spl_file_open (&stderr_file, filename,
#else
    ...
    if ((rc = apr_file_open_stderr(&stderr_log, stderr_pool))
        ...
        if ((rc = apr_file_dup2(stderr_log, stderr_file, stderr_pool))
                == APR_SUCCESS) {
#ifdef __VMS
            apache$spl_file_close(stderr_file);
#else
                     ...
                     int dummy_stderr)
{
    ...
    if (((rc = apr_procattr_create(&procattr, p)) == APR_SUCCESS)
        ...
        if (rc == APR_SUCCESS) {
#ifdef __VMS
        rc = apache$spl_proc_create(procnew, args[0], (const char * const *)args,
#else
...
#ifdef __VMS
    /*
    ** This allows the server to log to an IP Address & Port in the format of
    ** 'n.n.n.n:p', where 'n.n.n.n' is the IP Address and 'p' is the Port.
    */
    if (*s->error_fname == '@')
        {
        rc = apache$spl_socket_open (&s->error_log, s->error_fname + 1, p);
        if (rc != APR_SUCCESS)
            {
            ap_log_error (APLOG_MARK, APLOG_STARTUP, rc, NULL,
                          "Couldn't open error log socket '%s'",
                          s->error_fname + 1);
            return DONE;
            }
        }
...
#ifdef __VMS
        if ((rc = apache$spl_file_open(&s->error_log, fname,
#else
...
#ifdef __VMS
                    && strcasecmp(q->error_fname, virt->error_fname) == 0) {
#else
