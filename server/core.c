...
#include <fabdef.h>
#include "cvtfnm.h"
extern int decc$to_vms(const char *, int (*)(char *, int, void *), int, int, void *);
#endif
...
#ifdef __VMS
    if (apr$cvt_fnm (CVT_FNM_VMS_TO_UNIX, (char *) arg, (char **) &UnixFname))
        conf->access_name = apr_pstrdup (cmd->pool, UnixFname);
    else
#endif
...
#ifdef __VMS
    if (apr$cvt_fnm (CVT_FNM_VMS_TO_UNIX, (char *) arg, (char **) &UnixFname))
        arg = apr_pstrdup (cmd->temp_pool, UnixFname);
#endif
                                   ...
                                   const char *arg)
{
    ...
    if (apr$cvt_fnm (CVT_FNM_VMS_TO_UNIX, (char *) arg, (char **) &UnixFname))
        arg = apr_pstrdup (cmd->temp_pool, UnixFname);
    if ((apr_filepath_merge((char**)&ap_server_specific, NULL, arg,
                            APR_FILEPATH_TRUENAME, cmd->pool) != APR_SUCCESS)
        || !ap_is_directory(cmd->pool, ap_server_specific)) {
...
#ifdef __VMS
    if (apr$cvt_fnm (CVT_FNM_VMS_TO_UNIX, (char *) arg, (char **) &UnixFname))
        arg = apr_pstrdup (cmd->pool, UnixFname);
#endif
...
#ifdef __VMS
            if (strlen(r->filename) <= (PATH_MAX - 6)) {
                strcpy(tmp, r->filename);
            } else {
                goto error;
            }
            if ((t = strchr(tmp, ';'))) {
                if (strlen(t) <= 6) {           /* Maximum version is 32767 */
                    strcpy(fvn, t);
                    *t = '\0';
                    if (decc$to_vms(tmp, cb_to_vms, 0, 0, &t)) {
                        strcpy(tmp, t);
                        free(t);
                        if ((t = strchr(tmp, ';'))) {
                           strcpy(t, fvn);
                        } else {
                           strcat(tmp, fvn);    /* Probably an error */
                        }
                        filename = tmp;
                    } else {
                        goto error;
...
#ifdef __VMS
        if ((status = apr_file_open(&fd, filename, APR_READ | APR_BINARY
                            | (((r->finfo.fab_rfm == FAB$C_VAR) ||
                               (r->finfo.fab_rfm == FAB$C_VFC)) ? APR_CTX_REC : APR_CTX_STM)
#if APR_HAS_SENDFILE
...
#ifdef __VMS
        /* Can't tell the actual size of variable length and VFC
           format files without counting all the bytes.  So go
           count them and reset the value returned by stat().
           Not real efficient, but RMS caching should help some.
        */
        if ((r->finfo.fab_rfm == FAB$C_VAR) ||
            (r->finfo.fab_rfm == FAB$C_VFC)) {

        /* Do not force chunked encoding for VAR and VFC files */
        /*    r->chunked = 1;                                  */

            r->finfo.size = apr_vms_var_file_size(fd, r->finfo.size);
        }
#endif
