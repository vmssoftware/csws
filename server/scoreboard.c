...
#ifdef __VMS                    /* Ignore this on OpenVMS; BRC */
...
#ifdef __VMS
    apr_vms_shm_destroy(ap_scoreboard_shm);
#else
                                                ...
                                                const char *fname)
{
     ...
     * segment. */
#ifdef __VMS
    apr_vms_shm_remove(fname, pool); /* ignore errors */
#else
    apr_shm_remove(fname, pool); /* ignore errors */
#endif

#ifdef __VMS
    rv = apr_vms_shm_create(&ap_scoreboard_shm, scoreboard_size, fname, pool);
#else
...
#ifdef __VMS
        rv = apr_vms_shm_create(&ap_scoreboard_shm, scoreboard_size, NULL,
...
#ifdef __VMS
    if (ap_scoreboard_shm == NULL) {
        apr_status_t rv;
        void *sb_shared;
        ap_calc_scoreboard_size();
        rv = open_scoreboard(p);
        if (rv || !(sb_shared = apr_vms_shm_baseaddr_get(ap_scoreboard_shm))) {
            return HTTP_INTERNAL_SERVER_ERROR;
        }
// Do not memset() here as we hose what's already in the shared memory segment!
//      memset(sb_shared, 0, scoreboard_size);
        ap_init_scoreboard(sb_shared);
     }
#endif
#ifdef __VMS
    if (apr_vms_shm_size_get(ap_scoreboard_shm) < scoreboard_size) {
#else
...
#ifdef __VMS
        apr_vms_shm_detach(ap_scoreboard_shm);
#else
...
#ifdef __VMS
// >>> BRC
        ps->generation = ap_scoreboard_image->global->running_generation;
#else
