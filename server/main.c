...
// #ifdef __VMS
// #if APR_USE_VMSDLM_SERIALIZE
//     printf(" -D APR_USE_VMSDLM_SERIALIZE\n");
// #endif
// #endif
...
#ifdef __VMS
    ap_log_error(APLOG_MARK, APLOG_STARTUP, 0, NULL,
                 "       %*s [-k start|restart|graceful|graceful-stop|stop|flush]",
                 pad_len, " ");
#else
    ap_log_error(APLOG_MARK, APLOG_STARTUP, 0, NULL,
...
int main(int argc, const char * const argv[])
{
    ...
    while ((rv = apr_getopt(opt, AP_SERVER_BASEARGS, &c, &opt_arg))
        ...
        case 'd':
#ifdef __VMS
            if (apr$cvt_fnm (CVT_FNM_VMS_TO_UNIX, (char *) opt_arg, (char **) &UnixFname))
                ap_server_specific = apr_pstrdup (pcommands, UnixFname);
            else
                ap_server_specific = opt_arg;
#else
...
#ifdef __VMS
            if (apr$cvt_fnm (CVT_FNM_VMS_TO_UNIX, (char *) opt_arg, (char **) &UnixFname))
                confname = apr_pstrdup (pcommands, UnixFname);
            else
#endif
            confname = opt_arg;
            break;
...
#ifdef __VMS
    /*
    ** If we're in a control processing state, then call AP_RUN_MPM here
    ** and then exit cleanly.
    */
    if (ap_mpm_query (AP_MPMQ_IS_CONTROL_PROCESS, &rv) == APR_SUCCESS && rv)
        {
        ap_run_mpm (pconf, plog, ap_server_conf);
        destroy_and_exit_process (process, 1);
        }
#endif
