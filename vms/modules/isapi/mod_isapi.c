...
#ifdef __VMS
#pragma message disable (LVALUECAST)
#endif

/* Retry frequency for a failed-to-load isapi .dll */
#define ISAPI_RETRY apr_time_from_sec(30)

#define MAKELONG(a, b) ((long)(((short)((unsigned long)(a) & 0xffff)) | ((unsigned long int)((short)((unsigned long)(b) & 0xffff))) << 16))
                                        ...
                                        apr_size_t headlen)
{
    ...
    if (stat) {
        cid->r->status = ap_scan_script_header_err_strs(cid->r, NULL,
                                        (const char **) &termch, &termarg, stat, head, NULL);
        cid->ecb->dwHttpStatusCode = cid->r->status;
    }
    else {
        cid->r->status = ap_scan_script_header_err_strs(cid->r, NULL,
                                        (const char **) &termch, &termarg, head, NULL);
...
{
    apr_status_t rv;
#if APR_MAJOR_VERSION && APR_MINOR_VERSION > 4
