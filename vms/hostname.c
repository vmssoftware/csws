...
static void SetLogName(char *, char *);
static void SetSymName(char *, char *);
static void Usage();
...
*/
	...
	if (!strchr(hostname, '.')) {
		...
		if (strchr(hp->h_aliases[i], '.') &&
		    !strncasecmp(hp->h_aliases[i], hostname,
				 strlen(hostname))) {
...
*/
    if (argc - optind == 1) {
	char *addr_ptr = argv[optind], *addr_sep;
...
**
*/
static void SetLogName(char *LogName, char *LogValue)
{
    struct dsc$descriptor_s log_nam_desc =
	{ 0, DSC$K_DTYPE_T, DSC$K_CLASS_S, 0 };
    struct dsc$descriptor_s log_val_desc =
	{ 0, DSC$K_DTYPE_T, DSC$K_CLASS_S, 0 };
    int status;
...
**
*/
static void SetSymName(char *SymName, char *SymValue)
{
...
*/
    status =
	lib$set_symbol(&sym_nam_desc, &sym_val_desc, &LIB$K_CLI_LOCAL_SYM);
    if (!(status & 1))
	exit(status);
...
{
    fprintf(stdout,
	    "Usage: HOSTNAME [-l log-name] [-s sym-name] [host-addr]\n");
}
