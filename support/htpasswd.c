...
#ifdef __VMS
        if (apr$cvt_fnm (CVT_FNM_VMS_TO_UNIX, dirname, &UnixFname))
            dirname = strdup (UnixFname);
#endif
...
#else
        /* to allow close before copying */
        if (apr_file_open(&ftemp, dirname, APR_CREATE | APR_TRUNCATE | APR_READ | APR_WRITE | APR_BUFFERED,
