...
int main (int argc, const char * const argv[])
{
    ...
    struct dsc$descriptor MbXDesc = { MAXLEN, DSC$K_DTYPE_T, DSC$K_CLASS_S, MbXName };
...
#ifdef __VMS
    i = 0;
    ItmLst[i].BufLen = MAXLEN;
    ItmLst[i].ItmCod = DVI$_DEVNAM;
    ItmLst[i].BufAdr = &MbXName;
    ItmLst[i++].BufRLA =&MbXLen;
    ItmLst[i].BufLen = 0;
    ItmLst[i].ItmCod = 0;
    ItmLst[i].BufAdr = NULL;
    ItmLst[i++].BufRLA = NULL;

    // Acquire the MBX device name
...
#ifdef __VMS
    sts = sys$assign (&MbXDesc, &channel, 0, 0, 0);

    if(!(sts&1)) {
        lib$signal(sts);
    }

    SplMsgBuf = malloc(MSGLEN);
    memset(SplMsgBuf, 0, MSGLEN);
#else
    ...
    for (;;) {
        ...
        if (strlen(SplMsgBuf)<1)
        {
           sleep(3);
           continue;
        }
        sprintf(buf,SplMsgBuf); /* Why sprintf() ? */
        nRead = strlen(buf);
        memset(SplMsgBuf, 0, MSGLEN);
#else
        nRead = sizeof(buf);
...
#ifdef __VMS
        apr_file_flush(status.current.fd);
        fsync(status.current.fd->filedes);
#endif
        if (nWrite != nRead) {
