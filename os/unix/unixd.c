...
static int AddSecurityAce (char *, char *, char *, char *);
static int JobLogicalAce = 0;
#endif          /* __VMS */
                              ...
                              apr_pool_t *p)
{
    ...
    */
    ...
    if (! JobLogicalAce)
        {
        if (AddSecurityAce ("LNM$JOB", "LOGICAL_NAME_TABLE", SUEXEC_USER, "RWED") <= 0)
            ap_log_error (APLOG_MARK, APLOG_ERR, errno, NULL,
                         "AddSecurityAce: bad status returned %08X", vaxc$errno);
        else
            JobLogicalAce = 1;
        }
#endif
...
#ifdef __VMS
            /*
            ** Display this error message if we haven't received a CANCEL
            ** during this accept.  The cancel is most likely due to a
            ** shutdown or restart request.
            */
            if (! (status == EVMSERR && vaxc$errno == SS$_CANCEL))
#endif
...
#ifdef __VMS
static int AddSecurityAce(char *object, char *class, char *user,
                          char *access)
{
    $DESCRIPTOR(ObjectNameDesc, "");
    $DESCRIPTOR(ClassNameDesc, "");
    $DESCRIPTOR(UserNameDesc, "");
    ILE3 OssItems[2], *Ile3Ptr;
    ACEDEF ObjectAce;
    UICDEF Uic;
    int status;

    UserNameDesc.dsc$w_length = strlen(user);
    UserNameDesc.dsc$a_pointer = user;

    status = SYS$ASCTOID(&UserNameDesc, (unsigned int *) &Uic, 0);
    if (!(status & 1)) {
        decc$$translate(status);
        return (-1);
    }

/* Build the Object ACE for this user */

    ObjectAce.ace$b_size = ACL$K_LENGTH;
    ObjectAce.ace$b_type = ACE$C_KEYID;
    ObjectAce.ace$w_flags = 0;
    ObjectAce.ace$l_access = 0;
    if (strstr(access, "R") || strstr(access, "r"))
        ObjectAce.ace$l_access |= ACE$M_READ;
    if (strstr(access, "W") || strstr(access, "w"))
        ObjectAce.ace$l_access |= ACE$M_WRITE;
    if (strstr(access, "E") || strstr(access, "e"))
        ObjectAce.ace$l_access |= ACE$M_EXECUTE;
    if (strstr(access, "D") || strstr(access, "d"))
        ObjectAce.ace$l_access |= ACE$M_DELETE;
    if (strstr(access, "C") || strstr(access, "c"))
        ObjectAce.ace$l_access |= ACE$M_CONTROL;
    ObjectAce.ace$l_key = Uic.uic$l_uic;

    ILE3_INIT(OssItems);
    ILE3_ADD(OSS$_ACL_ADD_ENTRY, ObjectAce.ace$b_size, &ObjectAce, NULL);
    ILE3_TERM;

    ObjectNameDesc.dsc$w_length = strlen(object);
    ObjectNameDesc.dsc$a_pointer = object;

    ClassNameDesc.dsc$w_length = strlen(class);
    ClassNameDesc.dsc$a_pointer = class;

/* Add the ACE for this user */

    status = SYS$SET_SECURITY(&ClassNameDesc,
                              &ObjectNameDesc,
                              NULL, 0, &OssItems, NULL, NULL);
